...
\begin{gapbox}
The Phase 6 downlink block in Figure~\ref{fig:conops-state-flow} labels one of its outputs
``Cryobot insertion package'' --- language that reads as generating an actual deployment
package, whereas the mission's settled scope (REQ.06.02, the Executive Summary) is
\emph{identifying} a candidate insertion site, not producing hardware or a deployment plan for
one. The diagram is otherwise complete and ready to use; only that one label needs revising
(e.g., to ``Candidate site selection'') to stay consistent with the rest of the report.
\end{gapbox}
...
\begin{gapbox}
\textbf{Partially resolved.} The mobility solution's $17.1$~kg (Section~7.7: wheels, rocker
suspension, drive motors, mast pan/tilt actuator) is now carried as an explicit Mobility row in
...
cap, $12.35$~kg/$8.4\%$ margin). The corresponding $3.5$~W from the same design has \emph{not}
been added to the power budget (Table~\ref{tab:power-budget}, Section~7.4) -- it isn't yet clear
which operating mode(s) draw it or for how long, and the source spreadsheet still needs that line
item before the power table is fully reconciled.
\end{gapbox}
...
\begin{gapbox}
REQ.06.02, REQ.07.01, and REQ.08.03 have been checked directly against the authoritative
\textit{3.7 Command and Data Handling/Space Robotics Mission Sheet - Requirements.pdf}:
REQ.06.02's condensed entry above now carries the literal statement text (it was previously a
reconstruction that also dropped the one-sol response-time clause); REQ.07.01 and REQ.08.03
were already worded consistently with the master sheet and needed no change. REQ.07.03 remains
unresolved --- not a wording issue, but the $28.5$~kg vault allocation baked into its literal
``shall'' statement still does not match the $25.75$~kg Structure subsystem total in the mass
budget (see the gapbox in Section~7.3); that discrepancy requires a decision (revise the
requirement's number or grow the mass budget) rather than a transcription fix, and is left open
for the author.
\end{gapbox}
