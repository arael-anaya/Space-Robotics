...
completed source deliverable yet (e.g., real sampled data for the data product), and (2) numbers
that were reconstructed or paraphrased from adjacent documents rather than copied verbatim, most
of which have since been checked against the master spreadsheet and resolved. See also
Appendix~D for the itemized AI-use disclosure carried over from each source assignment.
\end{gapbox}
...
Table~\ref{tab:mass-rollup}. The master spreadsheet now states $25.75$~kg in the requirement's
``shall'' statement, matching the mass budget; the risk note and verification-plan text
attached to the same requirement still echo the old $28.5$~kg figure and are worth a pass for
full internal consistency, but the normative requirement itself is fixed.
