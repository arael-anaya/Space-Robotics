...
which read the existing assignments (1.1 through 3.7) and assembled, reorganized, and lightly
...
completed source assignment yet (the flight software state diagram, the full
requirements-verification status), and (2) numbers that were reconstructed
...
resolve this discrepancy before final submission.
\end{gapbox}
...
\begin{itemize}[leftmargin=*]
    ...
    any processor core, restoring nominal operations without ground intervention, and log each
    event to non-volatile memory with a timestamp and affected core identifier. (CDH, Electronics)
...
\begin{gapbox}
REQ.05.02's literal ``shall'' statement gives a $330$~s latch-up recovery bound, but its own
reasoning field, testing method, and verification/validation plan in the master spreadsheet all
state a $30$~s bound instead, and the completed CDH design below (Figure~\ref{fig:cdh-diagram})
was built to the $30$~s figure. This reads as a typo in the requirement statement (a dropped
digit) rather than an intentional order-of-magnitude relaxation, since every other field agrees
with $30$~s and the $44$-minute one-way light delay argument in the reasoning text is explicitly
built around keeping autonomous fault recovery fast. Confirm which figure is authoritative and
correct the requirement statement before final submission.
\end{gapbox}
...
\noindent\textbf{Study:} Analysis of the onboard flight computer and mass-storage architecture.
...
\noindent\textbf{Winning option: RAD750-class SBC with a $1$~GB rad-hard NAND storage board.}
...
(\textit{3.7 Command and Data Handling/Space Robotics Mission Sheet.xlsx}, the cumulative
requirements sheet) and is not fully duplicated here for length; representative full-text
entries appear in Sections~3.3 and 7.
...
Power, Structures \\
REQ.05.02 & Detect/recover from a processor latch-up autonomously, log event w/ timestamp \&
core ID (statement says $330$~s; reasoning/V\&V and the CDH design use $30$~s --- see
gapbox, Section~7.7). & CDH, Electronics \\
...
\begin{gapbox}
Entries above marked ``confirm literal text'' (REQ.06.02, REQ.07.01, REQ.07.03,
REQ.08.03) were reconstructed from context in adjacent documents rather than transcribed
directly from the master spreadsheet. Open \textit{3.7 Command and Data Handling/Space Robotics
